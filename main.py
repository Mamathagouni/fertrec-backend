...
def _has_iot(device_id: Optional[str] = None) -> bool:
    if device_id:
        return last_iot_data.get(device_id) is not None
...
DEFAULT_QA = [
    {
        ...
        "answer_en": "Yes. Urea is commonly used as a nitrogen fertilizer for rice in chemical farming. Apply it in 2-3 split doses and irrigate after application to reduce losses.",
        "question_te": "యూరియా వరి పంటకు సరిపోతుందా?",
        "answer_te": "అవును. యూరియా వరి పంటకు సాధారణంగా ఉపయోగించే నత్రజని ఎరువు. 2-3 విడతలుగా వేయండి మరియు తర్వాత నీరు ఇవ్వండి.",
    },
    {
        "id": "mop_cotton",
        "question_en": "is mop suitable for cotton on black soil?",
        "answer_en": "Yes. MOP can be suitable for cotton on black soil when potassium support is needed. Apply only as per recommended dose and avoid excess use.",
        "question_te": "నల్ల నేలలో పత్తికి MOP సరిపోతుందా?",
        "answer_te": "అవును. పొటాషియం అవసరం ఉన్నప్పుడు నల్ల నేలలో పత్తి పంటకు MOP ఉపయోగకరంగా ఉండొచ్చు. సిఫార్సు చేసిన మోతాదులో మాత్రమే వేయండి.",
    },
]
...
def compute_uncertainty_from_probs(top1: float, top2: float) -> Dict[str, Any]:
    ...
    note = "Final decision is yours. If uncertainty is HIGH, recheck SoilType/CropType and confirm NPK values."
    return {
        "level": level,
...
def _resolve_mode_values(req: FertilizerRequest):
    ...
    if mode == "IOT":
        ...
        if not _has_iot(req.device_id):
            raise HTTPException(status_code=400, detail=f"No IoT data found for device_id={req.device_id}")
        return (
    ...
    if mode == "OFFLINE":
        if manual_t is None or manual_h is None or manual_m is None:
            raise HTTPException(status_code=400, detail="Offline mode selected but Temperature/Humidity/Moisture were not provided.")
    ...
    mode_used = "OFFLINE" if used_manual else "IOT" if used_iot else "UNKNOWN"
...
def startup_checks():
    ...
    print("Firebase ready:", firebase_ready)
...
def get_latest_iot_summary():
    if not last_iot_data:
        ...
        return {**mock, "is_mock": True, "irrigation_advice": advice}

    latest_item = max(last_iot_data.values(), key=lambda x: x.get("timestamp", ""))
    advice = compute_irrigation_advice(latest_item.get("Moisture"))
    return {**latest_item, "is_mock": False, "irrigation_advice": advice}
...
def get_iot_by_device(device_id: str):
    ...
    return {**item, "irrigation_advice": advice}
...
def get_latest_cloud_data():
    ...
    try:
        ...
        for doc in docs:
            ...
            data["doc_id"] = doc.id
            return data
        return {"message": "No data found in cloud"}
    except Exception as e:
        return {"error": str(e)}
...
def get_default_qa():
    ...
    return {"defaults": DEFAULT_QA, "iot_data": latest_summary}
...
def crop_recommend(req: CropRequest):
    ...
    if req.explain:
        ...
        response["llm_explanation_en"] = run_llm(f"Why is {crop_name} suitable? Give 3 tips.", "en", ctx)
        response["llm_explanation_te"] = run_llm(f"{crop_name} ఎందుకు అనుకూలం? 3 సూచనలు ఇవ్వండి.", "te", ctx)
...
def fert_recommend(req: FertilizerRequest):
    ...
    if req.explain:
        ...
        base_response["explanation"] = (
            base_response["llm_explanation_te"] if language == "te"
...
def save_farmer_decision(req: DecisionSaveRequest):
    ...
    return {"message": "Farmer decision saved", "saved": entry}

@app.get("/decision/all")
def get_all_decisions():
    return {"count": len(farmer_decisions), "items": farmer_decisions}

@app.post("/disease/detect")
async def detect_leaf(file: UploadFile = File(...), explain: bool = Form(True)):
    try:
        ...
        if len(preds) != len(TOMATO_CLASSES):
            raise RuntimeError(f"Disease model output mismatch. Expected {len(TOMATO_CLASSES)} classes, got {len(preds)}")
